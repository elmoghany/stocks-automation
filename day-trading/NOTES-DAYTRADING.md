...
## MX-SERIES PRE-REGISTRATION (2026-09-02, written BEFORE any ranked run): MEAN-REVERSION ENTRY ON GAPPERS + TA-TRIGGERED SELLS IN THE 10:30-12:00 WINDOW
User direction (verbatim intent): "do mean-reversion entry on gappers,
but test different sell points, i.e. sell at around 10 am or 11 am --
not exactly, use trigger signals based on technical analysis with
target to sell within these ranges."

WHAT IS BEING TESTED. The IC study (IC-STUDY-honest-pool.md) measured
one tradeable corner: at 10:00, the LEAST-extended halal-PASS crosser
above $3, entered at the next print and held to the 15:00 flatten:
+2.52% mean / +2.26% median / 60% win over 432 days, gross, vs -0.47%
for a random pick. The IR-series only ever used gain_asc to RE-ORDER
the breakout triggers (with C37's exits, on a config later found
mislabeled). A true mean-reversion ENTRY -- buy the tightest crosser
at the clock, no breakout required -- has never been run through the
rotation harness, and the user's ask is the exit side of exactly that
trade: flatten inside 10:30-12:00, with a TA signal choosing the bar.

MACHINERY (all flag-gated, all default OFF; identity proofs below):
  day-trading.py simulate_trades:
    entry_mode="market_at_start": buy at the OPEN of the first bar with
      time >= entry_start (+slip, premarket spread haircut, same ticket
      sizing, 20%-of-10-min causal volume cap, halt-reopen guard, price
      band). The +10% rule 3 is NOT applied at entry: the candidate is a
      crossed gapper that may have pulled back, which is the thesis.
      The trigger machinery (ORB / PMH-break / dip-reversal) is bypassed.
    vwap_exit=True: sell at the close of the first post-entry bar that
      closes below session VWAP (typical price x vol, anchored 09:30;
      premarket entries anchor 07:00) after a post-entry bar closed
      above it.  rsi_exit=(14, 70): RSI(14) crosses down through 70.
      macd_exit=True: MACD(12,26,9) line crosses below its signal.
      rand_exit=(max_hold_min, tag): CONTROL, exit at entry + U{1..max}
      minutes. All evaluated on COMPLETED bars, filled at that bar's
      close through _sell_fill(kind="close") (clamped to [Low, High]).
    Candles.vwap_from(anchor), Candles.rsi_n(period) helpers.
  plan/rotation_sim.py: EXIT_KWARGS += vwap_exit/rsi_exit/macd_exit/
    rand_exit (whitelist, so every MX config resolves them explicitly
    on top of EXIT_HOLD; the HOLD echo assertion now allows exactly the
    ONE named rule and the run-end assertion allows exactly that rule's
    exit bucket + window-close). Per-config exit_end (flatten at the
    last bar < exit_end), sim_from (engine sees 07:00 onward so VWAP/
    RSI/MACD are warm at t0), min_px (last close <= t >= $3, applied
    BEFORE the control's shuffle). run_many(): one pass over the days
    shared by every config in a batch (bars, crossing, hygiene, halal
    gate are config-independent); run(cfg) == run_many([cfg])[cfg].
  PROOFS: plan/mx_exit_test.py (synthetic tapes -> expected exit bar
    for market_at_start incl. halt deferral and entry_cutoff, vwap_exit
    incl. the never-above case, rsi_exit, macd_exit, rand_exit
    reproducibility, target_pct gap-up fill; poison harness on 8 real
    days x 5 MX kwarg sets: 0/3,000 breaches). plan/fillmodel_test.py
    re-run (must still pass). Identity: C37F / HOLD1 / PTRAIL1 on
    --days 40 x 2 labels, pre-MX engine+harness (git HEAD worktree) vs
    post-MX, result rows AND trade dumps compared; plus the FULL C37F
    row inside the MX batch, which must reproduce -79,386 / -41,848
    (C37F-fm) to the dollar.

ENVIRONMENT: HALAL_STRICT=1 PT_FILED=1 POOL_HYGIENE=1 ROTTRADES=1
MASSIVE_TH_INTERVAL=0.25, slip=0.001 (10 bps/side), gap-through fills
(fill-model epoch a190a72), labels year + y2025, aug2026 out-of-sample.
Baseline to beat per ticket: C37F-fm -$56/ticket (-121,234 total).

CONFIG GRID (plan/rotation_sim.py CFGS, generated by _mx_cfgs):
  id = {MX|MT}{A|B}{1030|1100|1130|1200}{H|V|R|M|B|P|VB|T}[-R|-I|-S|-K3]
  MX = E1 market-at-start entry; MT = E2 existing breakout triggers from
  t0 (comparison). A = t0 10:00, B = t0 09:35. Window = exit_end; entry
  cutoff = exit_end - 30 min floored at t0 + 5 min (so the 10:30 window
  can place exactly one ticket at 10:00). Ranking gain_asc among crossed
  halal-PASS names, min_px $3, gap7 gates as C37, no stale-pick escape.
  Exits, each alone on EXIT_HOLD: H time-flatten only; V vwap; R
  rsi(14,70); M macd; B sell_mode bearish_always; P pressure_exit
  (10, 0.3, 'profit'); VB vwap + bearish_always; T fixed take-profit +5%
  (trail off so target_pct is reachable; comparison row).
  k=1 sequential (live cash rule) is primary; -K3 (topk=3) for the top-3.
  RUN ORDER: (1) MXA x 4 windows x 8 exits = 32 ranked + C37F identity,
  one batch; (2) MXB and MTA for the best 2 exit rules (by $/ticket
  summed over the 4 windows) = 16; (3) controls for every config that
  clears the per-ticket bar on both years: -R random pick x30 (ROTREP
  0-29, seeds rc60-{date}-{ticket}-{rep}, symmetric top gate), -I
  inverted ranking (gain_desc), -S shuffled exit timing (ranked pick,
  exit at entry + U{1..2 x mean hold} minutes, x30); (4) -K3 for the
  top 3; (5) aug2026 for every ranked config and the winners' controls.

PRIORS (stated so the result can contradict them):
  * H rows (time flatten only) are the cleanest test of the IC corner.
    Prior: per-ticket POSITIVE but small, and MONOTONE in exit_end
    (10:30 < 11:00 < 11:30 < 12:00) if the measured drift is real -- the
    window captures a fraction of the +2.52% gross to 15:00, minus 20
    bps round trip and the 50 bps premarket haircut where applicable;
    on $15k tickets that is roughly +$50..+$150/ticket. Honest doubt:
    the IC number is gross, on a pool without this harness's hygiene,
    and the halal-PASS cut rests on hundreds of observations.
  * TA exits (V, R, M) at the same window: prior WORSE per ticket than
    H. 1-min RSI/MACD/VWAP crosses on thin gappers fire on noise and
    sell the drift early; they may lower negm / maxDD. If a TA exit
    beats H at every window, that is the interesting finding.
  * B (bearish patterns, always): the XP-series' pattern exits were the
    largest positive column pre-backfill but under HOLD on the honest
    pool are unmeasured; prior: near H, more exits, lower per ticket.
  * P (pressure flip, 'profit' mode: sells only winners): prior worse
    than H (it can only cut winners short).
  * VB: between V and B.  T (+5%): prior near H; a +5% target hits on a
    minority of tickets and the rest flatten anyway.
  * MT (triggers from t0 with gain_asc): prior WORSE than MX -- buying a
    breakout of the least-extended name mixes two opposite signals, and
    IRGA (the re-ranked trigger config) lost.
  * MXB (09:35): prior worse than MXA -- the IC corner was strongest at
    10:00; 09:35 fills sit in the opening volatility.
  * Controls: -R prior about -0.5% per ticket (IC random -0.47% to
    15:00); -I must lose; -S should land near H of the same window
    (a random exit time inside the window harvests the same drift) --
    so a TA rule earns its keep only by beating -S, not just H.
PASS BAR (per config, all required): both years positive per ticket;
>= 90th percentile of its 30-rep random-pick control on total AND
ex_best; inverted control loses; adjacency coherent across the four
