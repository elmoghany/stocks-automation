"""Dump per-trade detail for S071 (C23 rules + exit window to 15:00) so
...
Writes data/massive/s071_trades_{label}.json (same shape as the C23 dump).
"""
...
SPEC = dict(x.BYID["S071"])          # C23 rules + exit_end=(15, 0)

for label in ("year", "y2025"):
    DUMP.clear()
    out = x.run_experiment(dict(SPEC), label)
    f = ROOT / f"data/massive/s071_trades_{label}.json"
